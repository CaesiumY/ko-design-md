...
**대조하다 md와 상류(핸드오프 번들)의 값이 어긋나면** 어느 쪽을 정본으로 삼을지는
아래 §「프리뷰 산문 감사 — 판정 근거의 등급」의 **"다만 상류가 늘 이기는 건
아니다"** 항이 정한다 — 그 항은 프리뷰 감사에서 나왔지만 **판정 기준 자체는 카탈로그 전반의 값
대조에 적용된다.**
...
**손실 전사**이기 때문이다 — **그 관계가 성립하지 않으면 적용되지 않는다.** 상류를
확인했는데 md와 다르면, 어느 쪽이 옳은지는 **어느 쪽이 브랜드 발행물에 더 가까운가**로
가른다. md가 브랜드 공개 문서를 값 단위로 대조해 적은 수치는 번들보다 강하다 —
번들도 재구성물이고, 브랜드 발행물과 어긋날 수 있다.

> **사례 (wanted 타입 스타일 수).** 번들 README는 `18`, md는 `19`로 적는다. md
> `:322`가 "몽타주 공식 타입 스케일 19행과 대응하며 **57개 값 중 56개가 일치**"
> `[src:5]`를 근거로 들고, 같은 문단에서 어긋나는 값 하나를 **"번들이 다르게 적은
> 값"** 으로 짚는다 — md가 이미 이 항목에 한해 번들을 덜 권위 있는 출처로 취급하고
> 있었다. "상류 우선"만 기계적으로 적용하면 **브랜드 공식값을 버리고 번들값으로
> 되돌리게 된다.**
