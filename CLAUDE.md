...
**손실 전사**이기 때문이다 — 그 관계가 성립하지 않으면 적용되지 않는다. **md가
브랜드 공개 문서를 값 단위로 대조해 적은 수치는 번들보다 강하다.** 실제 사례:
wanted의 타입 스타일 수를 번들 README는 `18`, md는 `19`로 적는데, md `:322`가
"몽타주 공식 타입 스케일 19행과 대응하며 57개 값 중 56개가 일치" `[src:5]`를
근거로 든다. **번들이 브랜드 발행물과 어긋난 경우**이고, md가 이미 그 차이를
"번들이 다르게 적은 값"으로 짚고 있었다. 상류를 확인했는데 md와 다르면, **어느 쪽이
브랜드 발행물에 더 가까운지**를 먼저 보라.
