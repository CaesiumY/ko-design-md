...
> **최종 상태는 3곳이 아니라 5곳, 파일당 캡션은 5개가 아니라 7개다.**
> 이 태스크는 계획대로 3곳을 달았고(`ccad2ef`), 이후 두 번의 리뷰 라운드가
> 두 곳을 더 찾아냈다 — 탭 건수(`6294ef8`, 표 캡션이 다른 섹션의 수치를 덮는다고
> 주장하고 있었다)와 07 NoticeList(`c05a500`, 스펙이 약속했는데 이 계획이
> 빠뜨렸다). **아래 Step 3 의 기대값 `5개` 는 이 태스크 직후 기준이며 최종
> 기준이 아니다.**
>
> 원문은 그대로 둔다 — 계획이 무엇을 놓쳤는지가 이 문서의 값어치다.
...
|---|---|
| 1 | masthead 식별 신호 캡션 |
| 2 | 허구 표시 데이터 캡션 3곳 (인기검색어 · 민원 처리 표 · 서비스 타일) — **최종은 5곳, Task 2 머리말 참조** |
| 3 | 무출처 사실 주장 9건 교체 |
| 4 | 09번 목업 폼 비인터랙티브화 |
| 5 | 푸터 3슬롯 치환 (삭제 0개) |
| 6 | `/preview/` 색인 차단 |
