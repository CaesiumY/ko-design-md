...
**전수 실측(2026-08-16, 세 문서를 다시 열어 프롭을 세었다):**
...
| 위치 | 이 절의 주장 | 실측 판정 |
| --- | --- | --- |
| `:357` | 오귀속 | ✅ **진짜 결함** — `[src:47]` 하나에 두 값을 귀속했으나 그 문서엔 `p={12}` 뿐 |
| `:207` | 오귀속 | ⚠️ **결함 아님** — `[src:47][src:26]` 합동 인용이 두 값을 실제로 덮는다. 짝이 교차돼 있을 뿐이다 |
| `:485` | 설명 부정확 | ❌ **정확하다** — 위 블록쿼트에서 철회했다 |
...
착지는 `docs/superpowers/specs/2026-08-16-class101-md-spacing-provenance.md` 가 갖는다.
