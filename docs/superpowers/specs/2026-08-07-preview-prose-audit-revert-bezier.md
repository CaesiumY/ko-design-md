...
- 대장: `2026-08-07-preview-prose-audit-false-positive-ledger.md` — **이 파일은 PR #256
  이 새로 만든다.** 이 저장소에는 아직 없으므로 #256 을 먼저 머지할 것.
...
**이 정정은 PR #256 의 대장에 이미 반영해 두었다**(그 PR 이 대장 파일을 새로 만들며,
bezier 절이 Divider 를 "뒤집힌 판정" 으로 싣는다). bezier 는 **되돌림 7 · 유지 2**
이고(유지 2 = 다크 Toast ·
