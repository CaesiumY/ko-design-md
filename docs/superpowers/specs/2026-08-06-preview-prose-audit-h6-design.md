...
**판정 단위는 노트가 아니라 주장이다.** `.code-card` 4개가 **15개** 주장을 담았고
(ActionButton 4 · ListItem 4 · BottomSheet 4 · Color 3) 그중 13개가 무근거였다.
근거가 있던 둘은 Color 의 `fg` · `bg` 로 md 의 역할 토큰 이름 그대로였다. 세는
단위는 **고친 표면**이다.

> **분모를 한 번 틀렸다.** 처음엔 16으로 적었다 — 카드 4장에 각 4줄을 곱한 수이고,
> Color 만 3줄이라는 걸 세지 않았다. 4회차 리뷰가 "표에서 16이 재현되지 않는다" 로
> 잡았다. **이 계열이 계수를 틀린 게 처음이 아니다**(H-1 172 · H-2 30 · H-3 93).
> 그래서 위에 카드별 내역을 적어 둔다 — 합계만 적으면 다음 감사가 이 수를 검산할
> 방법이 없다. 재발 방지는 "다음엔 잘 세자" 가 아니라 **검산 가능한 형태로 적는
> 것**이다.
...
**그 대체 문구 자체가 한 번 걸렸다.** `context` 줄을 `추가 정보를 제공` 으로 적었는데
md 는 `추가 정보나 액션 목록을 제공` 이다 — 바텀시트의 가장 흔한 용도인 액션 목록이
빠졌다. 무근거 주장을 지우는 편집이 근거를 좁히는 새 서술을 만든 것이고, 이 계열이
경계해 온 유형 그대로다. 3회차 리뷰가 잡아 `추가 정보나 액션 목록을 제공` 로 되돌렸다.
**대체 문구를 원문과 대조하는 일은 원문을 대조하는 일과 같은 무게로 해야 한다** —
이 문서가 그렇게 했다고 적어 둔 뒤에도 한 줄이 새어나갔다.
