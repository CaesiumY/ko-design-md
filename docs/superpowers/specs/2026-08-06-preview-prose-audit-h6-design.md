# 묶음 H-6 — seed-design 프리뷰 산문 감사

- 날짜: 2026-08-06
- 선행: #215(1차), #216(2차), #220(3차), #232(4차), #236(5차 — toss),
  **#231(seed-design 리뉴얼 — 제외 해제)**
- 대상: `public/preview/seed-design/{light,dark}.html`

> **순번은 6차다.** 착수 시점엔 5차였으나 작업 도중 #236(toss)이 5차로 먼저
> 착지했다. 순번은 착수 순서가 아니라 `git log --grep "프리뷰 산문 감사"` 의 착지
> 순서를 따른다 — 이 계열은 병렬로 진행되므로 착수 순서로 매기면 같은 번호가 두 번
> 나온다(실제로 났다).
...
브랜드 자산 리스크 감사 계열이고, 산문 감사로는 여섯 번째다(위 선행 목록).
seed-design 은 #209 가 "리뉴얼 준비 중"
...
잔여 12개 슬러그. **후보 개수는 상한이지 감사 단위가 아니다**(socar 14 → 33,
greeting 42 → 93).
...
**toss 는 #236 이 가져가 빠졌고, 그 결과가 이 표의 읽는 법을 정정한다** — 여기 적힌
수는 슬러그별 *대표 클래스 하나*의 후보 수이지 서술 표면 총수가 아니다. toss 는 이
표에 `caption` 6개로 올라 있었지만 #236 이 실측한 전체 표면은 54개였고 7개를
정정했다. 다음 슬러그를 이 수로 고르지 말 것.

**`wanted` · `bezier` · `line-design-system` 은 저작권 축만 판정됐다**(H-1). 서술
전수는 미착수이므로 7차 이후가 이 셋을 "완료" 로 취급하면 안 된다.
