...
`checkFile()` 안, `hero-logo-missing` push 직후·`scanCss()` 앞. 3가지를 따로 검사한다 —
클래스 존재(구조 앵커), `제휴·후원 관계가 없습니다`(나목), `더미 데이터`(허위의 사실).
문장 전체 매칭은 나중에 한 글자도 못 바꾸게 만들고, 클래스만 보면 빈 배너를 통과시킨다.
