...
## 정오표 — 캡션 열거가 화면의 허구 항목을 다 담지 못했다

최초 캡션은 각 화면을 눈으로 보고 쓴 열거였고, 그래서 빠진 게 있었다. 리뷰가
kyobobook 의 `베스트` 배지를 짚었고, 그걸 계기로 9곳을 화면 텍스트와 전수 대조하니
더 있었다.

| 위치 | 빠져 있던 것 |
|---|---|
| kyobobook | 평점 9.6·리뷰 2,481, `베스트` 배지와 "오늘의 베스트" 순위 |
| teamsparta | `국비지원` 배지 — **정부 지원과정 인증 주장** |
| gmarket 목업 | 송장번호 `6184-2057-3391` 과 택배사명(한진택배) |
| gmarket 아이템 | 쿠폰·사은품 혜택 표기 |
| toss 거래내역 | 가맹점명(카카오T·쿠팡), 거래 시각 |
| wanted | 근무지, 경력 조건 |

**패턴이 있다: 빠진 것 대부분이 자격·인증 주장이었다.** `공식`(gmarket)은 계획 단계에서
명시적으로 다뤘는데 같은 부류인 `베스트`·`국비지원` 은 놓쳤다. 실존 택배사 이름에 붙은
허구 송장번호도 마찬가지다. **교훈: "허구 데이터"를 수치로만 생각하면 배지·인증·순위처럼
값이 아닌 주장을 놓친다.** D-2 의 작성자 프롬프트에 자격 주장 배지 항이 들어가는 이유다.

## 정오표 — 문구 검사가 문서 전체를 뒤지고 있었다

`missing-disclaimer-banner` 의 문구 검사가 `html.includes(…)` 로 문서 전체를 봤다.
gmarket·kyobobook·teamsparta·toss·wanted 는 `.catalog-dummy` 캡션에도 `더미 데이터` 가
들어 있으므로, **상단 스트립에서 그 문장을 지워도 룰이 계속 통과했다.** 클래스와 문구가
서로 다른 요소에 흩어져 있어도 마찬가지였다.

스트립 요소의 내부 HTML 을 정규식으로 캡처해 그 안에서만 문구를 찾도록 고쳤다. 같이
클래스 매칭도 토큰 단위로 좁혔다 — `\b` 는 하이픈이 비단어 문자라
`page-catalog-disclaimer` 안에서도 매칭된다(이 저장소의 `hasAttrValue` 가 같은 이유로
`\b` 를 피한다).
