...
후보 7 → **되돌림 0 · 유지 1 · 상류미확인 6**. 종전 기록 `되돌림 4 · 유지 3` 은
**틀렸다**. 이 슬러그는 **집행할 것이 없다** — 아래.
...
**어느 것도 상류로 확인되지 않았다.** `DesignSync` 에 wanted 프로젝트가 확인되지
않고 크롤 캐시도 없다. `CLAUDE.md` 의 *"셋 다 없으면 결함으로 올리지 말고 상류
미확인으로 남긴다"* 를 적용한다.

**(1) SSOT 계열 4건 — 동일한 주장 두 쌍이다.** 종전 대장이 각 쌍을 되돌림/유지로
...
파일은 … 뿐"*)는 **다른 번들끼리 대조한 무효 논증**이다. **`(Community)` 를 둔 두
판정의 "충돌" 도 충돌이 아니었다 — 지시 대상이 다르다.** 하나는 신 번들 이름에 대해
참이고 다른 하나는 프리뷰 텍스트에 대해 참이다. **넷은 둘씩 묶여 같이 움직여야 한다.**

**(2) AVATAR `company` 변형 (1건).** 한때 되돌림으로 분류했으나 **근거가 부족했다.**
`.avatar.company{border-radius:12px}` 가 `f0ec0c2`(#86) 에서 처음 나타난다는 것은
**우리 저장소의 히스토리**일 뿐, 그 변형이 상류 번들에 있었다는 증명이 아니다.
md 는 avatar 컴포넌트군을 명명하되 이 변형을 문서화하지 않는다. **로컬 생성물의
계보를 상류 근거로 쓰는 것이 이 계열이 반복해 저지른 오류다.**
다만 **감사의 정정은 지금도 반쪽이다** — 캡션에서 `company` 만 지우고
`light.html:865`·`:870` 의 시연 2개는 남겨 화면이 계속 그 변형을 전시한다.
상류를 읽을 수 있게 되면 **캡션과 시연을 함께** 결정할 것.

**(3) eyebrow `350만+ 회원이 함께하는 커리어` (1건).** 한때 되돌림으로 분류했으나
**공개 출처가 뒷받침하는 범위를 넘는다.** 원티드 공개 페이지가 말하는 것은
`350만` **인재풀**이고, 정정 전 문구는 `350만+` **회원**이다 — `+` 하한 주장과
인재풀→회원 범위 전환이 **둘 다 출처에 없다.** 감사의 삭제 근거(*"프리뷰가 사업
통계를 발명했다"*)도 정확하지 않지만, 원문을 그대로 되살리면 **확대된 통계를
다시 발행**하게 된다. 되살릴 근거를 찾으려면 회원 수를 명시한 공식 출처가 필요하다.
