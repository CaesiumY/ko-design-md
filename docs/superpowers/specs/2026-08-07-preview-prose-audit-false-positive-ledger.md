...
| greeting | 14 + 미판정분 1 (Doodlin 워드마크 2건은 표면 없음) | #272 · #274 |
...
**실제로 손댈 것은 되돌림 14 + 미판정분 1 · 유지 9** 이다(미판정 1건은 2026-08-07 에 판정했다 — 아래).
...
#### 미판정이었던 1건 — 2026-08-07 에 오탐으로 판정, 되돌림 (#274)
...
**순수 md 침묵 논증이라 무효**다. 반대로 md 와 프리뷰 자신이 원문을 뒷받침한다 —
 이 "Toast 는 **반전 서피스**다" 를 적극 진술하고,
 의  이 / 를 **문자 그대로 공유**하며,
 주석이  로 지워진 주장과 같은 문장을
싣는다. 결정적으로 ** 는 바로 그 주석을 근거로 Toast 부제 정정을 정당화했다** —
같은 사실을 Toast 엔 채택하고 Tooltip 엔 부정한 자기모순이다.

**light.html 은 건드리지 않는다** — 감사 이전()부터 padding 문장이었고
 가 그 문장을 dark 에 복사한 것이다.
