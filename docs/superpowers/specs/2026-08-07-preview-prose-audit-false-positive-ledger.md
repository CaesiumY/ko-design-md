...
> 틀린다** — 각 슬러그를 집행할 때 **기각도 근거 등급으로 함께 재검토할 것.** 특히
> 경로·파일 이름을 근거로 든 기각은 그 경로가 **어느 트리** 것인지 먼저 확인해야 한다.
> 그리고 bezier Divider 가 보여주듯 한 판정 안에 **맞는 근거와 틀린 근거가 섞여 있을
> 수 있다** — 결론만 보고 통째로 채택하거나 기각하지 말 것.

> **기각 재검토 완료(2026-08-16).** 19건 전부를 사유 전문으로 재검토했다 —
> `2026-08-16-preview-prose-audit-rejection-recheck.md`. **집행 중 3건이 뒤집혔고**
> (bezier Divider #258 · wanted `1:1` 과 `(Community)` #281) 1건이 해소됐으며
> (wanted `18 styles` #282), 1건은 집행 불가다(greeting Doodlin 워드마크 — 카드가
> `584f363` 로 제거됨). **나머지 14건은 유지가 맞다 — 이번 스윕에서 뒤집힌 것은 0건이다.**
>
> **위 문단이 적었던 "16건이 부재 주장 형태" 는 표면을 센 것이었다.** 사유 안의 부재
> 어휘를 센 것이고, 실제로 무게를 지는 근거는 대부분 값·자기모순이다 — greeting `icons`
> 캡션은 **같은 카드의 note 와 정면 모순**이고, 다크 `Toast` 는 다크 렌더에서 거짓이며,
> `Drawer radius0` 은 md `:345` 가 **적극 금지**한 용도 매핑이고, `buttons 3 sizes` 는
> md Known Gaps 가 **명시 부인**한다. socar `Snackbar` 는 **번들이 자기 안에서 두 갈래로
> 렌더**하고, toss caveat 는 **삭제한 것이 없다**(이름 목록 무변경 + 캐비엇 덧붙임뿐).
> **기각 사유를 어휘로 분류하지 말 것 — 세어서는 알 수 없고 읽어야 한다.**
>
> **그리고 "참이다" 가 "되살려야 한다" 를 뜻하지 않는다.** greeting 브랜드 카드 2건은
> `greetinghr.com` 이 사실을 뒷받침하는데도(그리팅 ATS ×4 실재 · `Greeting HR` 0건 ·
> 기능명 8개 한글) 유지다 — **캡션은 규범문이 아니라 관찰문**이라는 별개 축이 남기
> 때문이다. 되돌리기가 "감사가 참인 것을 지웠다" 만 찾으면 이 축을 놓친다.
...
- **wanted 는 해소됐다**(2026-08-15). ⚠︎위치추정 4건은 `8cb74a6` diff 가 축자로
  짝지어 줬고, `SSOT … (Community)` 를 둔 "충돌" 도 충돌이 아니었다 — **지시 대상이
  다르고 양쪽 다 상류로 확인된다.** 프로젝트 이름은 `Wanted Design System`, 그
  프로젝트가 재구성하는 Figma 파일 이름이 `Wanted Design System (Community)` 다.
  아래 wanted 절 참조.
- **greeting 미판정 1건도 해소됐다**(2026-08-15) — 상류 `Tooltip.jsx` 주석이
  *"This follows the Toast surface (gray600 ground, 4px radius)"* 로 정정 전 문구를
  뒷받침한다. 아래 greeting 절 참조.
...
### wanted (원티드) — 2026-08-15 상류 확인 완료

후보 7 → **되돌림 6 · 유지 1**. 두 번 고쳤다 — 종전 `되돌림 4 · 유지 3`
도, 그 직후의 `유지 1 · 상류미확인 6` 도 틀렸다.

**`DesignSync` 에 `Wanted Design System`(`019e12e5-423a-7208-acb5-0bdbabdb7d89`)이
있다.** 없다고 판단해 6건을 상류미확인으로 남겼으나 실제로는 읽을 수 있었고,
**README.md 하나가 보류 전부를 축자로 답한다.**

#### 되돌린다 — 상류 README 가 축자로 담는다 (6건, #281)

| 표면 | 상류 근거 (`README.md`) |
| --- | --- |
| footer 고지 · `.strip-meta` — `SSOT: Wanted Design System (Community)` | 리드 문장이 *"Recreates the public **Wanted Design System (Community)** Figma file"*, Sources 표가 `"Wanted Design System (Community).fig"` |
| CSS 주석 · `.row-label` — `components-job-cards.html 1:1` | "Design System cards (preview/)" 목록에 `components-job-cards.html` 이 **그 이름 그대로** 있다 |
| `.eyebrow` `350만+ 회원이 함께하는 커리어` | CONTENT FUNDAMENTALS > Voice & tone: *"The marketing site routinely opens with **\"350만+ 회원이 함께하는…\"**"* |
| `.cl` `AVATAR · … company · 그룹` + `<span class="avatar company">W</span>` | 컴포넌트 표 Avatar 행: `.wavatar --{xs,sm,md,lg,xl}`, **`--company`**, `--brand` |

`.strip .strip-meta { display: none; }` 반응형 규칙도 함께 복원했다 — 감사가 산문과
같이 지웠다.

**`(Community)` 를 둔 두 판정의 "충돌" 은 애초에 충돌이 아니었다** — 지시 대상이
다르다. 그리고 **양쪽 다 상류로 확인된다**: 프로젝트 이름은 `Wanted Design System`
이고, 그 프로젝트가 재구성하는 Figma 파일 이름이 `Wanted Design System (Community)`
다. 프리뷰가 말한 것은 후자, 즉 **SSOT** 였다.

**되돌리기를 두 번 물렸다가 다시 넣은 자리다.** `350만+` 는 공개 마케팅 페이지가
`350만 인재풀` 이라 적어 "확대된 통계" 로 보고 철회했으나, **상류 README 가
`350만+ 회원` 을 그대로 인용**한다. AVATAR `company` 는 저장소 커밋 계보를 근거로
삼은 것이 부적절해 철회했으나, **상류가 정식 변형으로 발행**한다. 두 철회 모두
근거 등급 판단으로는 옳았고, **상류를 읽자 결론이 뒤집혔다.**

#### 재검토였던 1건 — 2026-08-15 해소, 감사가 옳았다 (#282)

- `<p class="value">7 × 19 named</p>` (현재 값, 감사가 `18` 에서 바꾼 것) — **유지한다.**

한때 **md↔상류 충돌**로 보고 유보했다. md `:322` 가 `총 19 styles`, 상류 README 는
두 곳에서 `18` 이라 적기 때문이다. **충돌이 아니었다 — 근거의 등급이 다르다.**

md `:322` 는 숫자만 적지 않는다:

> 총 19 styles. **이 표는 몽타주 공식 타입 스케일 19행과 대응하며, 57개 값 중 56개가
> 일치한다** — 본 문서에서 공개 출처와 가장 강하게 대조되는 정량 블록이다 `[src:5]`.

**md 의 자기 서술만으로는 부족하므로 `[src:5]` 를 직접 열어 확인했다** —
`montage.wanted.co.kr/docs/foundations/base-material/typography` 의 공식 타입 스케일
표는 **19개 명명 스타일**을 싣는다(Display 1·2·3 / Title 1·2·3 / Heading 1·2 /
Headline 1·2 / Body 1·2 × Normal·Reading / Label 1 × Normal·Reading · Label 2 /
Caption 1·2). md 의 표와 이름까지 일치한다.

즉 **번들 README 의 `18 named styles` 가 브랜드 발행물과 어긋난다.** md 는 같은
문단에서 어긋나는 값 하나를 "번들이 다르게 적은 값" 이라 짚어, 이 항목에 한해
번들을 덜 권위 있는 출처로 이미 취급하고 있었다.

**이 계열의 일반 규칙(상류 우선)이 뒤집히는 자리다.** 규칙이 상류를 앞세우는 이유는
md 가 번들의 **손실 전사**이기 때문인데, 여기서는 md 가 번들이 아니라 **브랜드 공개
문서**를 대조했다. 손실 전사 관계가 성립하지 않으므로 상류 우선도 적용되지 않는다.
...
`class101` 은 **2026-08-15 에 감사했다**(#287) — 캡션 12건 중 결함 1건을 고쳤고 11건은
이번 라운드에서 반증되지 않았다. 자세한 것은
`2026-08-15-preview-prose-audit-class101.md`. **잔여 없음** — 11건의 `[src:N]` 개별
대조를 `2026-08-16` 에 마쳤다(`2026-08-16-class101-caption-citation-crosscheck.md`).
**10건은 원문이 축자로 뒷받침했고 1건은 인용이 어긋나 고쳤다** — `lineLimit` 이
FilterChip 공식 문서 프롭 표에 없어 `[src:20]` 에서 떼어 MIT 저장소 `[src:4]` 로
옮겼다. **주장 자체는 참이었다**(`FilterChipProps.ts` 가 선언하고 스토리가 시연한다) —
옛 기준이었다면 "출처에 없다" 를 근거로 지워 참인 주장을 없앴을 자리다.
**공식 문서의 프롭 표가 전수가 아니라는 것**이 이 슬러그에서 나온 교훈이고, 다른
슬러그에도 적용된다.

**md 의 `px={20}` 오귀속(이슈 #288)은 `2026-08-16` 에 정정했다**(#289) —
`2026-08-16-class101-md-spacing-provenance.md`. 이슈가 든 3건 중 실제 결함은
`:357` 하나였고, **이슈 본문 자체가 검증 안 된 부재 주장을 담고 있었다**
(`[src:26]` 이 `px={20}` 을 담지 않는다고 적었는데 14회 담는다). 이슈대로
집행했다면 맞는 인용을 지웠을 것이다 — **이 계열이 반복해 밟은 함정이 이슈
본문에도 들어간 사례**이므로, 남은 대조를 이어받는 세션은 이슈·스펙에 적힌
부재 주장도 근거 등급으로 다시 볼 것.

**다만 class101 은 번들이 상류가 아니다.** md 가 공개 문서·MIT 오픈소스·npm 에서
직접 만들어졌고 Claude Design 프로젝트는 스스로를 그 공개 출처의 재구성물이라 밝힌다.
판정은 `vibrant-design.com` · `github.com/pedaling/opensource` · npm 으로 한다 —
자세한 것은 `CLAUDE.md` 의 「번들이 상류인 것은 md 가 번들에서 나왔을 때뿐이다」 항.
...
**재검토 완료(2026-08-16).** `@vapor-ui/core` 1.3.0 배포본과 공개 저장소로 대조했다 —
근거는 `2026-08-16-vapor-ui-components-upstream-crosscheck.md`.

- **`table th`** — 상류는 프리뷰의 `12px` 도 md 의 `13px` 도 뒷받침하지 않는다. th 는
  `subtitle1`(**14px / 500**)로 td 와 **같은 크기**다. 캡션 caveat 는 **유지가 맞고**
  근거만 바뀐다 — "md 가 13px 을 싣기 때문" 이 아니라 **"어느 쪽도 상류값이 아니기
  때문"** 이다.
- **툴팁 카피** — `삭제하면 복구할 수 없다/없습니다` 문자열이 상류에 **없다**(데모용
  창작). 존댓말화는 md `:31` 의 보이스 정책을 따른 것인데 **그 정책의 상류 근거를
  확인하지 못했다** — 코드 검색 0건은 약한 근거라 단정하지 않는다. **판정 유보**이고,
  문서 사이트 `[src:1]` 확인은 이슈 292번의 체크리스트에 있다.
  **→ 유보 해소(2026-08-17): 존댓말 캡션 유지가 맞다.** 문서 사이트를 열어 확인했고
  본문 카피가 존댓말이다(툴팁 문서 페이지가 컴포넌트를 *"요소에 대한 추가 정보를 제공하는
  툴팁 컴포넌트입니다"* 로 소개한다). 보이스 정책의 상류 근거가 있으므로 되돌리기가
  존댓말로 고친 것은 정당하다. **이로써 이 대장이 vapor-ui 에 열어 둔 건은 없다.**

**그리고 이 재검토가 더 큰 것을 찾았다.** `services/vapor-ui.md` 의 `## Components`
35개 항목이 상류와 계통적으로 어긋난다 — `13px` 8곳 · 무게 `600` 6곳 · `vp-*` 클래스가
1.3.0 배포본 553파일에 **0건**이다. 절 첫머리에 감사 메모를 넣었고(#293) 재저작은
이슈 #292 로 열었다. **`## Colors` · `## Typography` 는 공개 발행값과 대조돼 있으므로
손대지 말 것.**
