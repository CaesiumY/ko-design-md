...
후보 7 → **되돌림 6 · 재검토 1 · 유지 0**. 두 번 고쳤다 — 종전 `되돌림 4 · 유지 3`
...
#### 재검토 1건 — md 와 상류가 어긋난다

- `<p class="value">7 × 18 named</p>` → `19` (감사가 바꾼 것, 현재 값)

**md `:322` 는 `총 19 styles`, 상류 README 는 두 곳에서 `18`** 이라 적는다 —
`/Typography  7 hierarchy levels × 18 named text styles` 와
`7 hierarchy tiers … **18 named styles total**`. 즉 정정 전 `7 × 18 named` 는
상류 축자였고, 감사가 md 를 근거로 19 로 바꿨다.

**이건 md 침묵이 아니라 md↔상류 충돌이라 이 대장의 규칙 밖이다.** 어느 쪽이 맞는지
(번들이 낡았는지, md 가 다른 것을 셌는지) 가리기 전에는 손대지 않는다. 값을 되돌리면
md 와 어긋나고, 두면 상류와 어긋난다 — **문서 하나를 고쳐야 풀리는 문제다.**
