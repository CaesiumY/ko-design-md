...
| greeting | 15 (표면 없는 1 제외) | 미착수 — 통째 복원 불가 |
| wanted | 4 | 미착수 — 상류 재확인 선행 |
| gmarket · vapor-ui | 1 · 2 (상류 미확인 6 유지) | #257 되돌리기, 아래 |

PR 번호는 열린 시점 기준이다. **머지 여부는 `gh pr view <n>` 로 확인할 것** — 문서에
적힌 상태는 늘 낡을 수 있다.
...
- **vapor-ui `table th 12px` → `목업 표 th 12px는 이 프리뷰의 재현`** — md:551 이 th 를
  ...
  범위를 좁힌 것이다.
- **더미 캡션 3 곳**(멤버 표 · 콘솔 목업 · gmarket 컴포넌트 그리드) — 실명 자연인·실명
  서비스에 붙은 발명값 라벨이라 이 규칙의 대상이 아니다.

### 상류 미확인으로 남긴 것

이 되돌리기는 **상류를 읽지 못한 채 집행했다** — `DesignSync` MCP 미연결 ·
`.claude/cache/design-md/` 없음 · 공개 문서 미확보. 그래서 판정 기준을 **"근거가 md 침묵
뿐이었나"** 하나로만 썼다. 유지한 6 건 중 툴팁과 `table th` 는 md 가 적극 진술하므로
