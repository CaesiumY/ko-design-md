...
즉 md 의 19 는 **브랜드가 발행한 공식 타입 스케일을 값 단위로 대조한 결과**다.
번들 README 의 `18 named styles` 는 그보다 약하다. 결정적으로 **md 는 같은 문단에서
어긋나는 값 하나를 "번들이 다르게 적은 값" 이라 짚는다** — 이 항목에 한해 번들을
덜 권위 있는 출처로 이미 취급하고 있었다.
