...
| A | 라이선스·고지 문서 | ✅ |
| B | 그리팅 정합성 복구 | ✅ |
| C | 자산 정리 (seed-design 사진 교체, 로고 리사이즈) | ❌ |
| D | 프리뷰 고지 배너 34파일 | ❌ |
| E | KRDS 자산 중립화 | ❌ |
...
seed-design 사진 교체와 로고 리사이즈는 C에 남긴다.
...
- 접수 이메일 채널 추가 (A 잔여)
- seed-design 실사진 2장 교체, 로고 리사이즈 (C)
  - ⚠️ **seed-design 은 리뉴얼 예정**(2026-08-02 사용자 확인). 사진 2장 교체는 리뉴얼에 흡수될
    수 있으므로 **별도로 착수하지 말 것** — 리뉴얼 범위를 먼저 확인한다. 다만 초상권 축은
    라이선스 문서로 닫히지 않는 유일한 축이므로(CC BY §2(b)(1) 이 퍼블리시티·프라이버시를 명시
    제외), 리뉴얼이 지연되면 사진만 먼저 내리는 판단이 필요하다. 로고 리사이즈는 seed-design 과
    무관하므로 영향받지 않는다.
