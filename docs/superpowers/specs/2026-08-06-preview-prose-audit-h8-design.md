...
### 같은 실패를 내 수정에서 재현했다 — AVATAR

`.cl` AVATAR 캡션에서 `company` 를 지웠는데 **바로 아래 시연 마크업은 그대로 뒀다.**
`.avatar.company` 는 `border-radius: 12px` 인 둥근 사각형이고, md `### avatar` 는
아바타를 `radius-full` 하나로만 정의한다. 문구는 고쳤지만 화면은 여전히 md 에 없는
변형을 아바타의 한 형태로 전시하고 있었다.

**이 PR 이 바로 위에서 H-1 의 footer 를 두고 지적한 "부분 수정을 전체 수정으로
읽는다" 가 그 지적을 쓴 커밋 안에서 재현된 것이다.** 1회차 리뷰가 잡았다.

AVATAR 셀의 그 span 만 걷었다. **지워도 시연은 남는다** — 같은 파일 LIST CELL 셀이
leading 슬롯에서 같은 타일을 두 번 보여주고, md 에 실재하는 것은 `### job-card` 의
`company-logo-tile`(radius-12)이므로 거기가 맥락이 맞다. H-1 의 *"지우면 시연할 것이
사라진다"* 가 여기엔 걸리지 않는 이유다.

교훈은 "캡션을 고칠 때 마크업도 보자" 가 아니다 — **캡션이 열거하던 것을 지우면 그
열거가 가리키던 대상이 무엇이었는지 확인해야 한다.** 열거는 대개 아래 시연을 가리킨다.
