...
>
> 남은 하나는 `wanted` 인데, 원인은 토큰 펜스가 아니라 `## Components` 의 스펙 펜스
> 5개다 — 각각 0열 `height:` 를 갖고 린터가 그것을 최상위 스키마 섹션 중복으로 읽는다.
> 그 펜스는 사이드카에도 명세 토큰 맵에도 자리가 없어 본문에 남아야 하므로,
> **어댑터는 제거하지 않고 유지한다.** "md 자체가 스펙 문서" 는 16/17 에서만 참이다.
>
