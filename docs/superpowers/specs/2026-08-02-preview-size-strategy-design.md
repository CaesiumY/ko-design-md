...
오직 색을 보여주려고 존재하는 요소를 센다. **닫는 태그가 없는 void 요소**
(`img`·`hr`·`br` 등)도 따로 훑는다 — 짝맞춤 정규식에 걸리지 않아 통째로 빠지면
`<hr style="background:…">` 가 게이트의 사소한 우회 경로가 된다. void 요소는 내용을
가질 수 없어 정의상 언제나 fill-only 다.
...
**임계값**: `typoLabels >= 5` **그리고** `typoLabels >= 고유 typography 토큰 수 × 0.7`
이면 block. 분모는 중복 제거한 수를 쓴다 — 분자(`labelledTokenNames`)가 `new Set` 으로
중복을 지우므로 분모도 같아야 비율이 일관된다.

이름 매칭의 뒤쪽 경계에는 **한글 음절을 포함한다.** 이 카탈로그의 프리뷰는 전부 한국어
산문이고 `title1 은 60px` 이 아니라 `title1은 60px` 이 자연스러운 문형이라, 구두점·공백만
경계로 두면 규칙이 자기가 검사하는 언어의 기본 서법에서 눈이 먼다.
...
작은 시스템에서 우연한 언급 두어 개가 비율을 넘겨 오탐하는 것을 막는다.

**이 값은 실행 중 두 번 정정됐다.**

1. **절대값 6 → 비율+바닥.** 정리된 greeting 이 정확히 6 이라 자기 자신을 block 했다.
   남은 6개(`title7`·`body5`·`body6`·`item4`·`item5`·`item6`)를 열어보니
   `Line-height roles` 카드의 논지("`body5` 와 `item5` 는 둘 다 16px 이지만 배율이
   1.6em 과 1.5em")와 Fields·Button 컴포넌트 스펙이었다 — 루브릭이 **오히려 요구하는**
   적용 맥락이다.
2. **비율 0.5 → 0.7.** 0.5 는 타입 토큰이 10개 이하인 항목에 거짓 block 지대를 열었다.
   바닥 5 와 겹쳐 5개만 언급해도 걸리는데, 코퍼스 17개 중 5개가 그 구간이다
   (gmarket 7 · kyobobook 8 · line-design-system 10 · teamsparta 10 · seed-design 10 —
   29%). greeting 이 빠져나간 건 토큰이 22개였기 때문일 뿐이다. 0.7 은 시연된 참양성
   (100% 열거, 8-of-8, 7-of-7)을 모두 유지한다.
...
다만 PR-1 이후에도 greeting 18 은 코퍼스 최대값이 된다(2위 bezier). 임계값 24 는 이
현실을 수용하되 재발(82 수준)은 막는 위치다.

**bezier 의 실제 값은 7 이 아니라 12 다.** 위 표의 7 은 줄 단위 정규식이 센 수이고,
DOM 순회는 12 를 센다 — 같은 태그가 중첩된 요소(`<span class="av" …><span class="ic" …>`)를
non-greedy 매칭이 놓치기 때문이다. 판정은 어느 쪽으로도 24 아래라 바뀌지 않지만, 여유를
논할 때는 12 를 쓰는 것이 옳다. 이 차이는 코드 주석이 "과소 계수 방향이라 안전하다"고
적어 둔 한계의 실제 사례이며, `preview-validator-corpus.test.ts` 가 배포본 전수에서 두
측정의 **판정**이 어긋나는 순간을 잡는다.
