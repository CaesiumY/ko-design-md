...
| `.help` | `focus 보더는 **1.5px** blue-700.` | md 는 Focused 상태의 존재만 적고 **보더 두께도 표시색도 발행하지 않는다** → `상태 7종 중 Focused 다.` (아래) |

### 대체 문구가 또 무근거를 만들었다

`.help` 를 처음엔 `focus 상태는 **accent(blue-700)로 표시한다**.` 로 바꿨다.
`1.5px` 는 걷었지만 **색 주장을 새로 넣은 것**이다 — md 는 `:375` 에서 Focused 를
7종 상태의 하나로 적고 `:420` 에서 Dropdown Focused 가 PC 전용이라 적을 뿐,
**포커스 표시색을 어디에도 발행하지 않는다.** 2회차 리뷰가 잡았고
`상태 7종 중 Focused 다.` 로 다시 고쳤다.

> **이 계열이 다섯 번째로 같은 실패를 했다.** *"대체 문구도 원문과 같은 기준으로
> 검증한다"* 는 H-1 이 세운 규칙이고, 이 작업의 계획서에도 함정으로 적어 두었는데
> 그 문서를 쓴 세션이 그대로 저질렀다. **규칙을 아는 것과 자기 편집에 적용하는 것은
> 별개다** — 지운 쪽만 md 로 확인하고 넣은 쪽은 확인하지 않으면 감사가 아니라
> 교체다. 실무 형태로 적으면: **대체 문구를 쓴 직후 그 문구의 명사·수치를 md 에
> 다시 grep 할 것.**
...
### 부분 수정을 또 했다 — hover 흔적 5자리

`.panel-note` 를 `hover` → `pressed` 로 고치면서 **같은 데모 블록의 나머지를 그대로
뒀다.** 1회차 리뷰가 잡았다:

- `.demo-label` `State — **hover** / disabled` — 바로 위 캡션이 "pressed 다" 라고
  말하는데 라벨은 "hover 상태 데모" 라 **같은 화면에서 둘이 충돌**했다.
- 스크린샷 강제용 클래스 `.is-hover`(규칙 3 + 사용처 2)
- 다크 CSS 주석 `/* **hover**/press → lighter on dark */`

전부 `pressed` 로 맞췄다. **실제 `:hover` 셀렉터 4개는 그대로 뒀다** — 마우스 오버
동작은 브라우저의 것이고 md 가 금지하지 않는다. 고친 건 **md 용어로 상태를 이름
붙이는 자리**뿐이다.

> **이 계열의 "부분 수정을 전체 수정으로 읽는다" 가 두 PR 연속 나왔다.** 8차
> (wanted)에서는 캡션의 열거를 지우고 시연을 뒀고, 여기서는 캡션의 용어를 고치고
> 라벨·클래스를 뒀다. 8차가 얻은 교훈은 *"캡션이 열거하던 것을 지우면 그 열거가
> 가리키던 대상을 확인하라"* 였는데, **한 자리 넓혀야 한다** —
> **어떤 용어를 고치면 그 용어가 같은 블록에서 또 쓰이는지 grep 하라.**
> `hover` 로 한 번만 grep 했으면 다섯 자리가 다 나왔다.
