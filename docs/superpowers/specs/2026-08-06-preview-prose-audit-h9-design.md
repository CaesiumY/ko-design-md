...
- **`.badge` 가 여전히 빨간 것 — 무변경. 10차가 다시 열지 말 것.** 1회차 리뷰가
  캡션에서 `red fill` 을 뺐는데 데모 CSS(`background: var(--c-red-400)`)는 그대로임을
  짚었다. **8차 AVATAR 와 형태는 닮았지만 성격이 다르다** — 그때는 캡션의 *열거*가
  md 에 없는 **변형**을 가리켰고, 지우면 화면이 계속 무근거를 전시했다. 여기서는
  캡션이 색 주장을 놓았을 뿐이고 남은 문구(`count · Primary / Secondary 스토리`)는
  참이다. 게다가 md Don't 는 *"accent 는 **상태·카테고리 인코딩**에 쓰고"* 라고
  적으므로 카운트 배지의 빨강은 **md 가 승인하는 용법**이다. 데모는 어떤 색이든
  골라야 하고, md 가 색을 규정하지 않는다는 것이 "빨강이면 안 된다" 는 뜻은 아니다.
