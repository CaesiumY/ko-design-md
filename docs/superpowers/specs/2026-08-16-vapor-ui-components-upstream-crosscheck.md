...
> **표는 대표 항목 발췌다 — 대조 자체가 전수였다는 뜻이지 이 표가 36행이라는 뜻이 아니다.**
> md 의 `###` 항목 36개를 전부 읽고 비교했으나, 표는 `button` 의 variant 8개
> (`button-primary` · `button-danger-outline` …)를 `button` 한 행으로 묶고 값 주장이 없는
> 항목(`badge-solid` · `badge-soft` 등 색만 가르는 것들)을 뺐다. **재저작은 이 표가 아니라
> md 본문을 항목 단위로 훑을 것** — 표에 없는 행이 "대조했는데 맞았다" 를 뜻하지 않는다.
...
- **툴팁 카피** — `삭제하면 복구할 수 없다/없습니다` 문자열이 상류에 없다(데모용
  ...
  **→ 유보 해소(2026-08-17).** 문서 사이트를 열어 확인했다. 본문 카피가 존댓말이다 —
  툴팁 문서 페이지가 컴포넌트를 *"요소에 대한 추가 정보를 제공하는 툴팁 컴포넌트입니다"*
  로 소개한다 [src:1]. 즉 보이스 정책의 상류 근거가 있고 **존댓말 캡션은 유지가 맞다.**
  같은 조회에서 **문서 사이트를 수치 출처로는 쓸 수 없다**는 것도 확인됐다 — props 표가
  `Loading component documentation...` 로 지연 로드된다. 수치는 배포본이 정본이다.

## 재저작이 드러낸 이 문서의 오류 (2026-08-17)

재저작(이슈 292번)은 이 문서의 표를 정본으로 쓰지 않고 1.3.0 배포본을 다시 받아
컴포넌트 CSS 35개와 믹스인을 재추출했다. **이 문서가 예고한 대로 표만으로는 모자랐고,
표 자체에 틀린 행이 있었다.**

- **`icon-button` radius 를 `8px → 9999px` 로 적은 것은 오독이다.** `IconButton` 은
  `<Button className={cn(root(variantProps))}>` 로 **Button 레시피를 합성**하고, 자체
  `shape` 축의 기본값 `square` 클래스(`.icon-button-1ssqycy1`)는 **CSS 규칙이 아예 없다**.
  9999px 는 `circle` 변형 전용이다. 즉 **기본 radius 는 Button 에서 물려받은 8px 이고
  md 가 적던 8px 이 맞았다.** 이슈 본문도 이 오독을 그대로 실었다 — 이 문서가 스스로
  경고한 *"맞던 것을 틀리게 만든다"* 가 경고문 옆에서 한 번 일어났다. **variant 표를 읽을
  때 `defaultVariants` 를 함께 보지 않으면 옵션 값을 기본값으로 오인한다.**
- **`button` 의 "font-size 선언 0건(사이즈별 폰트 없음)" 은 절반만 맞다.** `button.css`
  안에 선언이 없는 것은 사실이나, 레시피가 size variant 마다 타이포 믹스인 클래스를 함께
  붙인다(`size=md` → `button-rdwa1t7 typography-1saa0cgi typography-1saa0cga`). 해석하면
  **`sm`·`md`·`lg` 가 14px / 500 / lh 22px, `xl` 이 16px / 500 / lh 24px** 다. 사다리가
  4단이 아니라 2단일 뿐 폰트는 있다. **컴포넌트 CSS 파일만 읽고 매니페스트의 동반
  클래스를 빼면 이 층이 통째로 안 보인다.**
- **`button` radius 는 사이즈와 무관하게 8px 하나다.** 이 문서의 표가 radius 열을 두지
  않아 md 의 "xl 만 `400`(12px)" 이 검사를 빠져나갔다. `button.css` 전체에 radius 선언이
  한 줄뿐이다.
- **`badge` 의 `solid` · `soft` · `dot` 는 존재하지 않는다.** 축은 `colorPalette` × `size`
  × `shape` 뿐이고 시각 처리는 한 가지(`background-{intent}-100` + `foreground-{intent}-200`)
  다. `solid` · `soft` 문자열은 배포본 전체에서 CSS 문법(`1px solid`)으로만 나온다.
- **`button` 의 축은 md 가 적은 "7 variants" 가 아니다** — `colorPalette` 6 × `variant` 3
  × `size` 4 이고, md 에는 `success` · `warning` 팔레트가 없었다.

**그리고 오염이 `## Components` 에서 끝나지 않았다.** 같은 방법으로 나머지 토큰 절을
대조하니 셋이 더 나왔고, 전부 **배포본과 문서 사이트 CSS 청크가 서로를 확인해 주는**
2026-07-29 팔레트 정정과 같은 기준으로 판정됐다.

| 절 | 어긋남 |
| --- | --- |
| `## Typography` | fontSize 13/13 은 정확하나 **lineHeight 는 13단 중 4단만 일치**(`size-075` 14/20 → **14/22**, `size-1000` 120/130 → **120/156**). 스타일 표 18행은 존재하지 않는 `vp-*` 이름에 또 다른 값을 얹고 있었다(`vp-h1` 32/48 vs `heading1` **38/56**). 무게 `600` 은 토큰에 없다 |
| `## Spacing` | space 사다리가 15단이 아니라 **17단**(`175` 14px · `225` 18px 누락), 위 세 단이 어긋남(`700` 64→**56** · `800` 72→**64** · `900` 100→**72**). `size-component-*` · `size-borderWidth-*` 토큰군은 **존재하지 않는다** — 컨트롤 높이의 출처는 `size-dimension-*` 이고 그 첫 단은 24px 이다 |
| `## Rounded` | 11단은 정확. 12번째로 실린 `size-borderRadius-circle` 은 **토큰이 아니다** — pill 은 `9999px` 리터럴이다 |
| `## Elevation & Depth` | `box-shadow-md` y 오프셋 2px → **4px**, 스크림 `.4` → **`.32`**. **모션 토큰(`motion-duration-*` · `motion-ease-*`)은 존재하지 않는다.** 상태 정책의 "fill 을 darker step 으로 swap" 도 틀렸다 — 전 variant 공통으로 `gray-900` 오버레이를 8% · 16% 로 올린다 |

**이슈 292번이 `## Typography` 를 "손대지 말 것" 으로 보호한 근거가 바로 이 대목에서
반증됐다.** 그 보호는 *"Typography 스케일은 토큰 원본과 완전히 일치한다"* 였는데, 일치하는
것은 fontSize 열뿐이었다. **보호 지정도 근거를 갖는 주장이므로 검사 대상이다.**
