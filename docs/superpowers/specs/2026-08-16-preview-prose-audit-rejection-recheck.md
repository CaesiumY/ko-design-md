...
## 결과 — 기각 19건 전수, 이번 스윕에서 뒤집힌 것 0건

**두 수를 구분한다.** 기각은 **19건**이고 아래 표가 그 전부다. 그중 4건은 이 스윕
이전에 이미 처리됐으므로(뒤집힘 3 · 해소 1) **이번에 새로 검토한 것은 15건**이고,
그 15건은 `집행 불가 1 + 유지 확정 14` 로 나뉜다. 표는 19건 전부를 싣는다.
...
| **유지 확정** | **14** | greeting 9 · socar 2 · line-design-system 1 · toss 1 · 11st 1 — 근거는 아래 표 |
...
공개 출처에 있는데 빠졌으므로, **유지 14건 중 근거가 가장 얇은 것**으로 기록해 둔다.
