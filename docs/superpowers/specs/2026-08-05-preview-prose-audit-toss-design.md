...
| md 에 없는 변형에 재현 표기 | 1 | `CHIP … removable` |
...
## "없다" 를 grep 으로 판정했다가 두 번째로 틀렸다

`CHIP · resting / active / brand · removable` 을 처음에는 **통째로 걷어냈다.** 근거는
"md:386 이 셋만 정의하고 removable 은 **렌더조차 되지 않는다**(마크업 0건)" 였다.

**렌더된다.** 같은 행 여섯 번째 칩이 그것이다:

```html
<span class="chip active">엄마 <span class="x">×</span></span>
```

내가 쓴 패턴은 `chip[^"]*removable|chip-x|chip close` 였는데 실제 클래스는 그냥 `x` 라
어느 쪽에도 안 걸렸다. **이 문서가 "리터럴 grep 0건은 근거가 아니다" 를 교훈으로
적으면서 같은 함정에 빠졌다** — 다만 이번 대상은 md 가 아니라 **프리뷰 자신의 마크업**
이었다.

CI 리뷰가 잡았고, 캡션은 지우는 대신 **재현 표기로 되돌렸다** — NEW 섹션과 같은 처리다.
지운 채로 두면 캡션이 렌더보다 *적게* 말해 사용자가 보는 것(4)과 캡션이 말하는 것(3)이
어긋난 채 남는다.

**교훈: "없다" 를 주장할 때는 검색어가 아니라 대상을 열어 봐야 한다.** 캡션 바로 아래
10줄을 읽었으면 보였다.
