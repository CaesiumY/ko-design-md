...
같은 커밋(99aae06)에서 로컬 5회 + CI 2회 측정했다. **텍스트 수치는 한 기계 안에서 전부
같았다** — 로컬 5회가 서로 같았고, CI 2회는 84행의 텍스트 열이 하나도 다르지 않았다.
이것이 정확값 절반이 기대는 전제다.
...
켠 사용자가 실제로 보는 화면이기도 하다. 처음에는 `NON_TEXT_SLACK` 을 2로 넓혀 쫓았는데,
8회 관측에서 폭이 네 값(26·27·28·29)으로 계속 늘어나 **움직이는 과녁**이었다. 선언 뒤
로컬 2회 측정: 테마당 **한 행**, 두 회차 동일, `opacityApprox` 는 false.

그래서 슬랙은 다시 **1** 이고, 그 1은 노이즈가 아니라 **질문의 성격** 때문이다 — 비텍스트는
무엇이 컴포넌트인지를 사람에게 남겨 두므로 애초에 정확값으로 조이지 않는다.
