...
| non-text 의 `measured` | **강제** — 플로어(기록값보다 내려가면 block, 올라가는 건 통과) |
...
가린다 — 수집기가 이미 밝힌 "거짓 양성은 독자에게 한 줄, 거짓 음성은 설문의 존재 이유"와
반대 방향이다.

**대신 프리뷰가 스스로 선언했다.** toss 에 다른 세 프리뷰(samsung-one-ui · codeit ·
class101)가 이미 쓰던 형태를 그대로 넣었다:
...
쫓았는데, 8회 관측에서 폭이 네 값(26·27·28·29)으로 계속 늘어나 **움직이는 과녁**이었다.
선언 뒤 로컬 2회·CI 2회 측정: 테마당 **한 행**, 전 회차 동일, `opacityApprox` 는 false.

**그래서 플로어에 여유를 두지 않는다.** CI 5회에서 toss 를 뺀 비텍스트 40행이 바이트 단위로
동일했으므로, 내려간 행은 노이즈가 아니라 커버리지 손실이다. 올릴 수 있는 상수를 남기지
않은 것도 의도다 — 손잡이가 있으면 CI 가 빨개졌을 때 "일단 넓혀보자"를 부르고, 그게 위
detour 였다. 비텍스트가 정확값이 아닌 **플로어**인 이유는 그대로다: 올라가는 수는 아무것도
주장하지 않기 때문이고(무엇이 컴포넌트인지는 사람에게 남는다), 내려가는 수만 "측정이 아직
닿는가"에 답한다.

**이 변경으로 toss 비텍스트의 `fail` 이 줄어든다. 팔레트를 고친 것이 아니다** — 이전의
미달은 `opacity: 0.28` 인 애니메이션 중간 프레임을 샘플한 결과였고, 이제 정지 프레임(점이
불투명한 상태)을 잰다.
...
전수의 `borderline` 271행을 `services/{slug}.md` frontmatter `colors:` 와 대조했다. 측정된
...
**결과: 271행 중 181행이 두 색 모두 발행 토큰인 쌍이다.**
...
이유로 보인다. 평면 집계로 "271행 중 181행이 못 고치는 것"이라 읽지 말 것.
