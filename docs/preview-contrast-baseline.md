...
| non-text 의 `measured` | **강제** — 플로어(기록값 −2 미만이면 block) |
...
흔들린 것은 `toss` 의 비텍스트 두 줄뿐이다. **관측 폭은 두 행이다** — 로컬에서 dark
`measured` 26·27·28, CI 에서 dark 28 다음 26, light 26 다음 28. 같은 실행에서 두 테마가
반대로 움직이므로 한쪽으로 치우친 편향이 아니라 위상 샘플링 노이즈다. 그래서
`NON_TEXT_SLACK` 은 **2** 다 — 처음에 1로 뒀다가 아무도 건드리지 않은 CI 재실행이 막히는
것을 보고 실측으로 올렸다. 이 줄에 대한 warn 은 회차마다 뜨는 것이 정상이다.
...
`NON_TEXT_SLACK = 2` 가 흡수한다. 플로어가 행 둘을 봐주어도 그 존재 이유는 안 깎인다 —
수집기가 한 부류의 표면에 못 닿게 되면 행은 둘이 아니라 수십 개가 사라진다.
