...
> **살아있는 제품 스펙이다.** 최종 갱신 2026-09-26. 2026-05-06 에 쓴 V0 방향성 PRD(`d4c0086`)를 지금의 카탈로그에 맞춰 다시 썼고, V0 문서는 git 히스토리에 남아 있다.
...
- **가져가는 길은 셋이다.** ① 사이트에서 한 번에 복사해 붙여넣기(주 경로) ② 소비자 스킬 `use-design-md` ③ 기계가 읽는 엔드포인트 — 카탈로그 색인, 항목별 DESIGN.md(명세 파일명 주소 포함), 정규 URL 의 마크다운 협상.
...
24. 디자이너로서, 소개 페이지에서 신뢰 근거와, 항목의 DESIGN.md 가 명세 문서이자 카탈로그 항목인 이유를 확인하고 싶다. 그래야 받은 파일을 어디에 넣을지 안다.
...
29. 코딩 에이전트로서, 같은 항목을 명세의 파일명(`/services/{slug}/DESIGN.md`)으로 받고 싶다. 그래야 명세만 아는 도구에 바로 넣는다.
...
4. **한 표현.** 항목의 DESIGN.md 는 인용·프로비넌스까지 담은 그대로 발행하고, 변환본을 따로 두지 않는다(ADR 0003·0008).
5. **기계 엔드포인트.**
   ...
   | `/services/{slug}/DESIGN.md` | 같은 DESIGN.md, 명세의 파일명 |
...
10. **출처.** 출처는 누구나 열 수 있는 공개 자료만 쓰고, 인용은 References 의 번호를 가리킨다. 출처 목록은 References 한 곳에 둔다 — frontmatter 에 따로 적던 중복 목록은 걷어냈다(ADR 0004).
