...
- 항목 파일명은 `_`로 시작할 수 없습니다 — 사이트는 그런 파일도 항목으로 싣기 때문에 `pnpm validate:catalog`가 막습니다(초기의 `_demo-*` 데모 픽스처는 제거됐습니다).
...
- **브랜드 자산** (`public/logos/*`, `public/preview/*/assets/*`): 각 권리자 정책 — [NOTICE](./NOTICE) 참조
