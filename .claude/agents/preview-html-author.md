...
- Light tokens in `:root`; dark tokens in a second `<style>` scoped to
  ...
  last and nothing after it — not even a sheet inside `<noscript>`: the file's
...
- Prose that is true of only one theme goes in a
  ...
  - The node in front of the template and the first node the template renders
    must both be elements of the same kind: the same tag, and a class in common
    when both carry classes. Keep the wording inside those elements, never as
    bare text beside them — a swap replaces exactly one node, so text standing
    in front of a template, or text a template opens with, cannot be swapped
    cleanly, and the deterministic gate refuses it. A swap is defined by the
    node in front of it, so if you delete the light element, delete its
    template too or turn it into an insert; otherwise the swap takes whatever
    now stands in front of it (in gs-shop it took the demo `<div>` a trimmed
    caption used to precede) and dark loses that block. The gate catches that
    when the node now in front is a different kind of element. It cannot tell
    when a sibling of the same kind moves into place, so check what stands in
    front of every template you keep.
...
These previews render inside a catalog iframe — on phones AND on the detail page, where the main column is **~976px** (the `max-w-[1400px]` page minus a 280px sidebar + gap). So you must prevent horizontal overflow at **every** width, not just the 375px phone case. **Test at 375px, at the ~976px embed width, AND at each multi-column layout's narrowest state** — the width just _above_ each `@media` collapse breakpoint, where the cells are tightest. Overflow hides in those intermediate multi-column widths, not only at 375px: a 3-tab segmented control that fit a 1-column phone cell still overflowed every 4-column desktop cell (viewport ~901–1134px) and shipped, because the guard only checked 375px. Two root causes recur: **(a)** a CSS Grid `1fr` track flooring at its content's `min-content` and pushing past the container, and **(b)** an atomic `inline-flex`/`flex` control group (segmented control, toggle/button group) whose `white-space: nowrap` children cannot wrap or shrink. A third failure never overflows at all, so neither the width sweep nor any static check reports it: a wrapped row whose last item is alone on its row and `flex-grow`s to the full row width. Follow these rules for every layout you write:

- **Build multi-column demo rows — cards, tiles, product/job/course items — with CSS Grid, not `flex-wrap`.** Use `grid-template-columns: repeat(auto-fit, minmax(<min>px, 1fr))` — or, when the design is meant to hold exactly N columns, a fixed count `repeat(N, minmax(0, 1fr))` with its mobile collapse rule, which also keeps an incomplete last row on equal tracks — never `display: flex; flex-wrap: wrap` with `flex: 1 1 <basis>` on the children. When the item count does not divide by the column count, `flex-grow` leaves the last item alone on its row and it absorbs the whole row's free space — a card that is 213px wide on the full rows becomes 663px, and a fixed-ratio thumbnail inside it balloons with it. Grid gives every item the same track width in every row, and that is what fixes the orphan — an incomplete last row keeps its tracks. `auto-fit` only matters when the whole row holds fewer items than fit on one line: it then collapses the empty tracks and stretches those few items across the row. So when the row holds only a few items with a fixed-ratio thumbnail, use `auto-fill` instead, which keeps the empty tracks and the tiles' width (gmarket's `.items-row` does this with 3 cards). `flex-wrap` stays right for tag/pill rows and button groups, where a wide last item is harmless. The hardened form `minmax(min(<min>px, 100%), 1fr)` is fine too — the static scan reads through the inner call.
...
- Put a real control inside a `role="img"` mockup — assistive tech reads the mockup as one flat picture, yet a `<button>`, `<a href>`, form field or `tabindex="0"` there still takes a Tab stop that announces nothing, and a widget role (`role="switch"`) promises keyboard operation the picture cannot give. Render mockup controls (zoom buttons on a map, toggles in a settings screen) as `<span aria-hidden="true">` styled to look the same. Stage 9a2 warns on any focusable control inside a `role="img"` mockup.
