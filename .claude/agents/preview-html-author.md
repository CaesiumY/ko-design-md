...
- **Build multi-column demo rows — cards, tiles, product/job/course items — with CSS Grid, not `flex-wrap`.** Use `grid-template-columns: repeat(auto-fit, minmax(<min>px, 1fr))`, never `display: flex; flex-wrap: wrap` with `flex: 1 1 <basis>` on the children. When the item count does not divide by the column count, `flex-grow` leaves the last item alone on its row and it absorbs the whole row's free space — a card that is 213px wide on the full rows becomes 663px, and a fixed-ratio thumbnail inside it balloons with it. Grid gives every item the same track width in every row. `auto-fit` collapses the tracks a short row cannot fill, so the wide widths keep their column count; when the row holds only a few items with a fixed-ratio thumbnail, use `auto-fill` instead so a nearly-empty row cannot stretch the tiles (gmarket's `.items-row` does this with 3 cards). `flex-wrap` stays right for tag/pill rows and button groups, where a wide last item is harmless. The hardened form `minmax(min(<min>px, 100%), 1fr)` is fine too — the static scan reads through the inner call.
