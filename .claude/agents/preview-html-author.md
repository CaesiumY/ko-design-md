...
- Put a real control inside a `role="img"` mockup — a `<button>`, `<a href>`, form field, `tabindex="0"` or widget role (`role="switch"`) there still takes Tab focus while assistive tech reads the mockup as one flat picture, so the focus stop announces nothing. Render mockup controls (zoom buttons on a map, toggles in a settings screen) as `<span aria-hidden="true">` styled to look the same. Stage 9a2 warns on any focusable control inside a `role="img"` mockup.
