...
`## Do's and Don'ts` must carry at least one brand-specific **domain-boundary Don't**. When frontmatter has a `design_system_name` (or `name`/`slug` is itself a design system — e.g. KRDS, or a slug ending in `-design`, `-design-system`, `-ds`), it must *also* carry a **vendor-neutrality Don't** forbidding the system's own name / package names / class prefixes in the consumer's generated UI (per stitch-format.md §9). A missing vendor-neutrality Don't on such an entry is a `warn` issue.
