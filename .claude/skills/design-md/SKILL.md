...
- **`KNOWN_SPEC_LIMITATIONS`** (`src/lib/spec-limitations.ts`) — only when the entry publishes a `%` radius, which the
  published spec cannot express (a multi-stop gradient goes in the `gradients:` map instead, where it raises nothing). It is
