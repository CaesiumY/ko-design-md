...
- **병합 전에 기계 게이트를 통과해야 합니다.** CI가 frontmatter·섹션 순서·인용 번호가 실재하는 출처를 가리키는지·토큰 사이드카·프리뷰 구조를 검사하고, DESIGN.md 명세의 공식 린터도 돌립니다. 명세가 표현하지 못하는 브랜드 발행값(`50%` radius)은 버리지 않고 알려진 한계로 기록합니다.
