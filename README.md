...
값을 가져다 쓰려면 그게 정말 그 브랜드의 값인지 되짚을 수 있어야 합니다. 그래서 무엇을 되짚을 수 있고 무엇은 그럴 수 없는지를 문서에 드러냅니다.

- **공개 자료로 뒷받침되는 주장에는 출처 번호가 붙습니다.** 본문의 `[src:N]`은 문서 끝 `## References`의 N번째 자료를 가리키고, 출처는 누구나 열어 볼 수 있는 공개 자료만 씁니다. 번호가 없는 문장은 그 자리에서 공개 출처로 되짚을 수 없으니, 값을 그대로 가져다 쓸 때는 인용이 붙었는지부터 보세요.
- **모르는 것은 모른다고 적습니다.** 항목마다 `## Known Gaps` 절을 두고, 확인하지 못한 공백과 철회한 주장을 거기 모읍니다. 브랜드 발행물과 다시 대조한 결과는 이와 별개로, 그 값이 있는 섹션(예: `## Colors`) 첫머리에 날짜와 함께 남겨, 값을 읽기 전에 그 단서를 먼저 보게 합니다.
- **색은 OKLCH로 적습니다.** 항목이 옮겨 온 브랜드 hex가 있으면 곁에 병기하고, 둘이 맞는지를 기계가 대조합니다. hex가 병기되지 않은 토큰은 이 대조를 거치지 않습니다.
- **병합 전에 기계 게이트를 통과해야 합니다.** CI가 frontmatter·섹션 순서·인용 번호가 실재하는 출처를 가리키는지·토큰 사이드카·프리뷰 구조를 검사하고, DESIGN.md 명세의 공식 린터도 돌립니다. 명세가 표현하지 못하는 브랜드 발행값(`50%` radius, 다중 스톱 그라디언트)은 버리지 않고 알려진 한계로 기록합니다.
