...
> **대조 결과(2026-08-02).** 이 절의 색 토큰 33개 중 **29개가 공식 Color 페이지가 발행하는 hex와 ΔE ≤ 0.02**로 일치했다 [src:3]. 나머지 4개(`primary-pressed`·`service-money` 등)는 그 페이지 텍스트에 값이 없어 대조 대상이 아니었을 뿐이며, 네 토큰 모두 자기 병기 hex와 정합한다(`audit:oklch` 0 mismatch). 출처 27개는 전부 서버 렌더라 브라우저 없이 재검증할 수 있고, 최다 인용 출처가 11건(전체의 8%)으로 단일 출처 의존이 낮다.
