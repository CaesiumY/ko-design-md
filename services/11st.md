...
rounded:
  ...
  radius-full: 50%   # avatar · 명세 Dimension 밖(%)이라 공식 린터 에러, 값은 유지
elevation:
...
Chip은 항상 `{rounded.radius-pill}`을 사용하고, Button과 Badge는 `{rounded.radius-sm}` 또는 `{rounded.radius-md}` 범위에 둔다. Bottom Sheet의 상단 모서리만 `{rounded.radius-sheet-top}`(16px)을 적용해 시트가 화면에서 떠오르는 감각을 만든다.
...
// background: {gradients.brand-gradient}
...
Pressed 상태는 fill을 `{colors.primary-pressed}`로 어둡게 처리한다. 트랜지션은 `duration-fast` + `ease-standard`.
...
// border-radius: {rounded.radius-pill}
...
화면 하단 3~5개 아이콘 메뉴 [src:20]. 컨테이너 높이 `64`(그림자 포함), 아이콘 사이즈 `36` 고정. `{elevation.shadow-tabbar}`가 적용되어 상단 엣지가 살짝 떠 보인다 [src:20]. Selected 탭은 아이콘에 브랜드 그라디언트가 들어간 Filled 아이콘으로 전환된다 [src:6].
...
Overflow Menu의 Modal 타입으로, scrim(`{colors.bg-overlay}`)이 뒤 콘텐츠를 비활성화한다 [src:21]. 좌우 상단 패딩 `16`, 상단 코너 `{rounded.radius-sheet-top}` [src:21].
...
// box-shadow: {elevation.shadow-toast}
// border-radius: {rounded.radius-md}
```
...
- 브랜드 그라디언트(`{gradients.brand-gradient}`)는 **Speciality Button과 active 탭/홈 아이콘 Filled variant**에만 한정해 사용한다 [src:13].
