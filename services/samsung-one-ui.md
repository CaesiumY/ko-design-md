...
선택 모드에는 항목을 고른 뒤 Edit 버튼을 누르거나 항목을 길게 눌러 들어가며, 들어가기 전에는 다른 컨텍스트 메뉴를 띄우지 않는다 [src:9]. 들어가면 앱바와 툴바가 함께 선택 모드로 바뀌고 **선택한 개수를 앱바에 숫자로** 보이며, 앱바의 라디오 버튼/체크박스를 누르면 전체가 선택된다 [src:9]. 툴바는 Edit으로 들어갔으면 즉시, 길게 눌러 들어갔으면 손가락을 뗀 뒤 띄우고, 연속 스크롤로 고르는 동안은 숨기며, **선택한 항목이 없으면 띄우지 않는다** [src:9]. 2019 가이드라인 p.54–55 `16. Selection control` 기준이다.

### slider

정해진 범위 안에서 값을 바꾸는 컨트롤로 볼륨·화면 밝기 등에 쓰며, 값이 바뀌면 즉시 피드백을 준다 [src:9]. 색은 `{colors.primary}` role이다 [src:7]. 설정 화면의 기본형은 Normal · Focus · Click · Disabled 네 상태를, 볼륨 팝업과 퀵 패널 밝기용 선택형(Optional)은 Normal · Click 두 상태를 도식으로 발행한다 [src:9]. 2019 가이드라인 p.36 `06. Slider` 기준이다.
...
토스트에 동작 버튼을 붙여 토스트 내용과 관련된 동작을 바로 실행하게 하는 변형이다 [src:9]. 다른 토스트처럼 정해진 시간이 지나면 사라진다는 점에서 버튼을 누를 때까지 남는 스낵바와 다르다 [src:9]. **닫기·완료·확인처럼 팝업을 닫기만 하는 버튼은 넣지 않고, 동작은 2개를 넘길 수 없다** [src:9]. 2019 가이드라인 p.50 `13. Action toast` 기준이다.
