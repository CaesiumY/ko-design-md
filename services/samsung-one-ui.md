...
colors:
  ...
  black-dark: oklch(0.134 0 0)   # #080808 · Dark 테마 팔레트의 Black(Light 테마는 순흑) · One UI 2(2019) 발행값
spacing:
...
다크 모드는 권장이 아니라 기본 제공 장치다. 다크 모드에서 배경·메뉴·UI 요소는 검정 또는 짙은 회색으로 전환되어 눈부심을 줄이고 [src:8], 출시 전 라이트·다크 두 모드 모두에서 테스트해야 한다 [src:8]. 다크 테마 팔레트의 Black은 순흑이 아닌 `{colors.black-dark}`로 발행돼 있다 [src:9]. 같은 가이드는 베젤이 검은 디바이스에서 검은 배경이 베젤과 화면의 경계를 지워 화면이 시각적으로 확장돼 보이게 한다고 적는데 [src:9], 이것은 어두운 배경 일반의 근거이지 근흑을 순흑보다 앞세우는 규칙은 아니다. 모든 앱이 사용자가 원하는 시점(또는 예약 시각)에 다크 모드로 전환할 수 있게 하는 것이 권장된다 [src:9].
...
면이 있는 버튼. **회색 배경은 중간 강조, 유채색 배경은 높은 강조**를 나타낸다 [src:16]. 높은 강조 배경은 `{colors.primary-dark}`(다크 테마에서 `{colors.primary-dark-on-dark}`) [src:7], radius는 `{rounded.radius-button}` [src:16]. **한 화면에는 한 가지 버튼 스타일만 쓴다** [src:16] — 강조 등급은 색으로 구분하되 스타일 체계 자체를 섞지 않는다.
...
특정 작업의 진행을 보여 준다. **진행 상태와 완료 시점을 알면 막대(progress bar), 완료 시점을 모르면 원(progress circle)** 을 쓴다 [src:9]. 막대는 화면 전체를 덮기보다 앱바 같은 분리된 영역에 두어 현재 작업을 방해하지 않게 한다 [src:9]. 사용자 동작에 도착 화면이 있으면 앱바·툴바 같은 기본 요소를 먼저 보이고 갱신 중인 콘텐츠 영역에 원을 띄우며, 그러기 어려우면 방금 누른 버튼 위에 표시한다 — **화면을 덮는 진행 팝업은 피한다** [src:9]. 2019 가이드라인 p.45–47 `10. Progress indicator` 기준이다.
...
선택 모드에는 옵션 메뉴의 편집(Edit) 버튼을 누르거나 항목을 길게 눌러 들어가며 [src:9][src:39], 들어가기 전에는 다른 컨텍스트 메뉴를 띄우지 않는다 [src:9]. 들어가면 앱바와 툴바가 함께 선택 모드로 바뀌고 **선택한 개수를 앱바에 숫자로** 보이며, 앱바의 라디오 버튼/체크박스를 누르면 전체가 선택된다 [src:9]. 툴바는 Edit으로 들어갔으면 즉시, 길게 눌러 들어갔으면 손가락을 뗀 뒤 띄우고, 연속 스크롤로 고르는 동안은 숨기며, **선택한 항목이 없으면 띄우지 않는다** [src:9]. 2019 가이드라인 p.54–55 `16. Selection control` 기준이다.

### slider

정해진 범위 안에서 값을 바꾸는 컨트롤로 볼륨·화면 밝기 등에 쓰며, 값이 바뀌면 즉시 피드백을 준다 [src:9]. 색은 `{colors.primary}` role이다 [src:7]. 기본형(Default)은 Normal · Focus · Click · Disabled 네 상태를, 볼륨 팝업과 퀵 패널 밝기용 선택형(Optional)은 Normal · Click 두 상태를 도식으로 발행한다 [src:9]. 2019 가이드라인 p.36 `06. Slider` 기준이다.
...
토스트에 동작 버튼을 붙여 토스트 내용과 관련된 동작을 바로 실행하게 하는 변형이다 [src:9]. 다른 토스트처럼 정해진 시간이 지나면 사라진다는 점에서 버튼을 누를 때까지 남는 스낵바와 다르다 [src:9]. **닫기·완료·확인처럼 팝업을 닫기만 하는 버튼은 넣지 않고, 동작 버튼은 하나만 둔다** [src:9][src:39]. 영문판 문구는 "More than 2 actions cannot be provided"라 2개까지로 읽히지만, 같은 쪽 [Don't] 도식이 버튼 2개짜리 토스트를 위반 예로 들고 한국어판은 "2개 이상의 Action버튼은 제공하지 않음"이라 적는다. 2019 가이드라인 p.50 `13. Action toast` 기준이다.
...
- 작은 텍스트는 배경과 **4.5:1 이상**, 큰 텍스트(일반 18dp 초과 또는 볼드 14dp 초과)는 **3:1 이상**의 대비를 가져야 한다 [src:29]. 2019 가이드라인은 이를 크기 구간별 표(13dp 이하는 4.5:1, 14·17dp는 일반 4.5:1/볼드 3:1, 18dp 이상은 3:1)로 발행했다 [src:9]. WCAG에서 유래한 기준을 삼성이 공식 채택해 발행한 값이다.
...
6. **다크 모드는 발행 다크 팔레트로** — 다크 배경은 `{colors.black-dark}`(다크 팔레트의 Black을 배경에 쓰는 것은 [src:8]의 검정 배경 서술을 옮긴 해석), 두 테마를 항상 함께 생성한다.
...
10. **없는 값은 없다고 말한다** — line-height·weight 숫자·터치 타겟 **크기**·그림자 수치를 요구받으면 공개 발행값이 확인되지 않음을 밝히고 제품 자체 결정으로 명시한다. 컴포넌트 폰트 크기 9단과 터치 대상 간격 8dp는 발행돼 있으므로 이 목록에 넣지 않는다.
