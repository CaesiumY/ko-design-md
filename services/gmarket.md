...
heading-1: 24 / Gmarket Sans / 400 / 페이지 단위 타이틀 (홈 상단 메시지)
heading-2: 22 / Gmarket Sans / 400 / 페이지 단위 타이틀 (MyG 상단 메시지)
heading-3: 20 / Gmarket Sans / 400 / 템플릿 단위 타이틀 (홈 템플릿 타이틀)
heading-4: 18 / Gmarket Sans / 400 / 템플릿 단위 타이틀 (헤더 타이틀)
...
대표적으로 제목은 20px, 본문은 14px이다 [src:32]. 헤딩 4종은 `{typography.font-heading}`(Gmarket Sans), 본문 3종(`body-1`·`body-2`·`detail`)은 `{typography.font-body}`(Noto Sans KR)를 쓰며, 본문은 Regular 또는 Bold를 취한다 [src:32]. 줄 높이 토큰은 공개 자료에 명시값이 없어 별도 정의하지 않는다 — `(no published line-height token surfaced)`.

**헤딩 weight 1차/2차 소스 불일치.** 공식 타이포그래피 표(1차)는 Heading 1~4의 weight를 모두 Regular로 기재하나, 재현 CSS 토큰 파일(2차)은 헤딩에 Medium(500)을 적용한다 [src:32][src:34]. 위 토큰 블록은 1차 소스인 공식 표를 따라 `400`으로 기록하되, 다운스트림은 두 값 중 하나를 제품 근거 위에서 확정해야 한다.
...
페이지당 1회만 쓰는 키 액션 버튼이다 [src:11]. CTA보다 낮은 위계의 주요 행동에 쓰며, 한 화면의 primary slot은 하나만 둔다 [src:11]. Button 타입(CTA·Primary·Secondary) 중 Primary에 해당한다 [src:11].
...
장바구니·알림 개수를 표시하는 카운터 뱃지다 [src:9]. 색상은 중요도에 따라 `{colors.cta}`(장바구니·알림 동선) 또는 `{colors.green-500}`(Primary)를 쓰며, 마이페이지 한정 스마일클럽 쿠폰 개수에는 `{colors.smile}` 컬러를 쓴다 [src:9]. 장바구니 담기 인터랙션은 Lottie로 화면 중앙에서 0.2초 후 헤더 뱃지 위치로 이동하며 숫자를 카운팅한다 [src:9].
...
상품 리스트 필터링 전용 컴포넌트다 — Basic Filter Chips / Curation Filter Chips / Basic Option Chips 3종이다 [src:12]. 칩 하나당 최대 10자(22byte)이고, 폭을 초과하면 가로 스크롤한다 [src:12]. Active 시 해당 화면의 카테고리 색상(`{colors.sub-teal-500}` 등 Sub Color)을 적용한다 [src:12].
...
페이지 내 동일 위계 콘텐츠를 그룹화한다 — Tabs(섹션 2~3개, 높이 48px)와 Tabs Group(섹션 4개 이상·레이블 유동, 높이 50px)이 있다 [src:25]. 선택 시 Selection Indicator(하단 라인)가 새 탭으로 이동하며, 레이블과 숫자를 결합하면 숫자는 `{colors.green-500}`로 표시한다 [src:25].
...
- **헤딩 weight 1차/2차 소스 불일치.** 공식 타이포그래피 표는 Heading 1~4 weight를 Regular(400)로 기재하나 재현 CSS 토큰 파일은 Medium(500)을 적용한다 — 제품 근거 위에서 한 값으로 확정해야 한다 [src:32][src:34].
...
- **줄 높이·자간 토큰 부재.** 타이포그래피 컴포넌트의 size·family·weight는 확인되었으나 line-height·letter-spacing 토큰은 surfaced되지 않았다 [src:32].
