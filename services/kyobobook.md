...
- **OKLCH 변환값의 출처가 값마다 다르다(2026-08-02 정정).** 원본 시스템은 hex로 게시하지만, Color 페이지가 **텍스트로** 내보내는 값은 코어 4색뿐이다 [src:3] — 그 넷은 ΔE 0.001로 대조됐고(Colors 절 참조), 나머지 램프 값은 스와치 이미지 판독에 의존한다. "모든 색상이 공개 hex를 변환한 것"이라는 종전 서술은 이 구분을 지워 철회한다.
