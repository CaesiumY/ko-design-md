...
1. Claude Design 번들 `project/README.md` (시각 기반, 보이스/톤 규칙, 컴포넌트 분류, 아이콘 규칙)
2. Claude Design 번들 `project/colors_and_type.css` (모든 CSS 커스텀 프로퍼티: 정확한 hex 값, 타입·라운드·스페이싱·섀도우 토큰)
3. Claude Design 번들 `project/SKILL.md` (보이스/팔레트 앵커 매니페스트)
4. Claude Design 번들 `project/ui_kits/krds-website/index.html` (sample portal: identifier strip, header, hero, tiles, notice list, modal, toast)
...
12. Claude Design 번들 `project/preview/*.html` (38개 컴포넌트 개별 스펙 프리뷰 라이브러리: table, tabs, alert, critical-alert, form-controls, text-input, select, date-input-calendar, badge, accordion, pagination, breadcrumb, side-navigation, step-indicator, infobox, bottom-sheet, tooltip, chip-tag 등)
13. Claude Design 번들 `project/index.html` + `project/ui_kits/{login,signup,civil-application-form,application-complete,board-list,board-detail,mypage,customer-center,error-pages,mobile}/index.html` (contact sheet + 11개 페이지 패턴 합성)
