...
sources:
  - https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ
...
KRDS는 중앙행정기관·공공기관·지방자치단체가 공유하는 단일 시각 언어로, 신뢰·안정·접근성을 핵심 정서로 삼는다 [src:1][src:6]. 톤은 "informational, neutral and respectful — not marketing" — 정보 전달과 절차 안내가 목적이며, 마케팅적 과장이나 유희적 표현은 배제한다 [src:1]. v1.0.0 핸드오프 번들은 38개 컴포넌트 프리뷰와 11개 패턴 페이지(포털 홈·로그인·회원가입·5단계 민원 신청 폼·신청 완료·게시판 목록/상세·마이페이지·고객센터·오류 페이지·모바일 홈)를 갖춘 포괄적 UI 킷으로 제공되며, 단일 컴포넌트부터 완성된 화면 흐름까지 동일한 토큰 체계 위에서 조립된다 [src:1][src:5][src:8].
...
KRDS v1.0.0 번들은 컴포넌트를 두 층위로 제공한다. ① sample portal `ui_kits/krds-website/index.html`이 정부 포털 홈을 조립하고(아래 `government-identifier-strip`~`footer`) [src:4], ② `preview/*.html` 38종이 각 컴포넌트를 개별 스펙 카드로 노출한다 [src:1][src:12]. 모든 컴포넌트는 동일한 색·타이포·라운드·포커스·모션 토큰을 공유한다.
...
`{colors.primary-60}`(hover)보다 한 단계 더 어두운 `{colors.primary-70}`로 swap한다. transform·scale은 쓰지 않는다 [src:1].
...
비활성은 opacity로 흐리지 않고 평면 색 전환으로 처리한다 — `{colors.bg-subtle}` (`{colors.gray-10}`) 배경 + `{colors.fg-4}` (`{colors.gray-50}`) 텍스트, cursor `not-allowed`. opacity-only 비활성화는 지양하며(KRDS는 평면 색 전환을 선호한다), 이 규칙은 색상 변형과 독립적으로 적용된다 [src:1].
...
너비 480px, 흰 표면, `{rounded.radius-xlarge}`, `{spacing.space-8}` (32px) 패딩, `{elevation.shadow-4}`. 240ms 슬라이드업, 뒤에 `oklch(0.16 0.003 264 / 0.5)` 스크림 [src:4][src:1]. 헤더는 행동 질문형(예: "민원 신청을 시작하시겠습니까?"), 본문은 절차형 `합니다` 정중체(예: "본인 인증이 필요한 서비스입니다. 간편인증 또는 공동인증서로 본인 확인 후 진행됩니다.") [src:4].
...
위 포털 조립 컴포넌트에 더해, v1.0.0 번들은 다음 컴포넌트를 개별 프리뷰 카드로 제공한다. 모두 동일한 토큰·2px `{colors.primary-50}` 포커스 아웃라인·150–200ms `ease-out` 모션 규약을 공유한다 [src:1][src:12].

### input-text

높이 48px, 패딩 0 14px, `{rounded.radius-small}` (6px), 1px `{colors.border-default}` 보더. Hover에서 보더가 `{colors.fg-3}`로 짙어지고, focus에서 `{colors.primary-50}` 보더 + 2px `{colors.primary-10}` 링으로 전환된다. 에러는 `{colors.danger}` 보더 + helper text, 비활성은 `{colors.bg-subtle}` (`{colors.gray-10}`) 배경 + `{colors.fg-4}` 텍스트로 평면 처리한다 [src:12].

### textarea

`{component.input-text}`와 동일한 보더·포커스·에러 어휘를 다행 입력에 적용한다. 리사이즈는 세로 방향만 허용한다 [src:12].

### select

높이 48px, 우측 16px에 셰브론. 열림 상태 드롭다운은 `{colors.primary-50}` 보더 + `{elevation.shadow-2}`, 선택 항목은 `{colors.primary-10}` 배경 + 체크 표시를 단다 [src:12].

### date-input-calendar

입력 48px(우측 캘린더 아이콘) + 320px 캘린더 패널(1px `{colors.border-default}`, `{rounded.radius-medium}` (8px), 패딩 20px). 7열 요일 그리드에서 선택일은 `{colors.primary-50}` 배경 흰 텍스트, 기간 선택 범위는 `{colors.primary-10}` 배경, 일요일은 `{colors.danger}`, 당월 외 날짜는 `{colors.fg-4}`로 표시한다 [src:12].

### form-controls — radio · checkbox · toggle

라디오는 20px 원(2px `{colors.border-strong}`, 선택 시 내부 10px `{colors.primary-50}` 점). 체크박스는 20px 사각형 `{rounded.radius-xsmall}` (4px), 선택 시 `{colors.primary-50}` 채움 + 흰 체크. 토글은 44×24 pill로 off `{colors.gray-30}` / on `{colors.primary-50}`, 20px 흰 노브가 좌→우로 슬라이드한다 [src:12].

### chip-tag

chip(필터)은 pill 형태, 1px `{colors.border-default}`, 패딩 6/12. 활성 시 `{colors.primary-10}` 배경 + `{colors.primary-50}` 보더 + 체크, 제거형은 우측 X 원을 단다. tag(라벨)은 `{rounded.radius-xsmall}` (4px), 13px bold, semantic 색만 사용한다 [src:12].

### file-upload

1px `{colors.border-default}` 점선 드롭존(hover 시 `{colors.primary-50}`), 업로드 후 파일명·용량·삭제 버튼을 가진 첨부 행 목록으로 전환된다 [src:1][src:12].

### breadcrumb

13px, 좌측 16px 홈 아이콘, `›` 구분자는 `{colors.fg-4}`. 링크는 hover 시 `{colors.primary-60}` 밑줄, 현재 위치는 `{colors.fg-1}` bold로 고정한다 [src:12].

### pagination

40px 정사각 아이템 `{rounded.radius-small}`. 현재 페이지는 `{colors.primary-50}` 배경 흰 텍스트 bold, hover는 `{colors.gray-10}` 배경, 비활성(처음/끝)은 `{colors.fg-4}` + pointer 제거 [src:12].

### tabs

underline 변형은 패딩 14/20, 활성 탭에 3px `{colors.primary-50}` 언더라인 + `{colors.primary-60}` 텍스트(기본 `{colors.fg-3}`), 카운트 뱃지는 `{colors.gray-10}` pill. pill 변형은 4px 컨테이너 안에서 활성 탭을 흰 pill + `{elevation.shadow-1}`로 띄운다 [src:12].

### side-navigation

240px 폭, 1px `{colors.border-default}` 우측 보더. 링크는 3px 투명 좌측 보더를 두고, 활성 시 `{colors.primary-10}` 배경 + `{colors.primary-60}` 텍스트 + 3px `{colors.primary-50}` 좌측 보더 + bold로 전환된다. 그룹은 셰브론 토글로 접힌다. 페이지 내 목차(in-page-navigation)는 동일 어휘의 앵커 목록으로, 활성 앵커를 `{colors.primary-60}`로 강조한다 [src:12].

### step-indicator

36px 원(2px `{colors.border-default}`)을 커넥터 2px로 잇는다. 완료 단계는 `{colors.primary-50}` 채움 + 흰 체크, 현재 단계는 `{colors.primary-10}` 배경 + `{colors.primary-50}` 보더 + `{colors.primary-60}` 텍스트, 완료 구간 커넥터는 `{colors.primary-50}`로 채워진다. 라벨은 13px bold. 다단계 민원 신청 폼의 진행 표시에 쓰인다 [src:12][src:13].

### skip-link · top-button

skip-link는 평소 숨겨졌다가 키보드 포커스 시 `{colors.primary-50}` 배지로 노출되어 본문으로 건너뛴다(접근성 필수). top-button은 우하단 원형 버튼으로 페이지 최상단으로 스크롤한다 [src:1][src:12].

### table

헤더 셀 48px / 본문 행 40px, 셀 패딩 14/12. 헤더 하단 룰은 1px `{colors.secondary-70}` (`#052B57` — `{colors.primary-80}`와 동일 hex), 행 디바이더는 1px `{colors.gray-20}`. 상태는 좌측 컬러 보더가 아니라 인라인 뱃지로 표현한다(진행중 `{colors.primary-50}`, 완료 `{colors.success}`, 반려 `{colors.danger}`) [src:12].

### structured-list · text-list

structured-list는 라벨/값 2열 정의 목록으로 행마다 1px `{colors.gray-20}` 디바이더를 둔다. text-list는 bullet · ordered · check 3종이며, check 목록은 `{colors.primary-50}` 체크 아이콘을 머리표로 쓴다 [src:12].

### accordion · disclosure

accordion은 상단 2px `{colors.fg-1}` 룰 + 항목 하단 1px `{colors.gray-20}`, 헤더 17px bold(hover `{colors.primary-60}`), 24px 셰브론이 열림 시 180° 회전한다. disclosure는 단일 "더 보기" 토글로 본문 일부를 점진 노출한다 [src:12].

### carousel

가로 슬라이드 모듈. 하단 도트 인디케이터(활성 `{colors.primary-50}`)와 좌우 화살표 컨트롤을 두며, 자동 재생은 사용하지 않는다 [src:1][src:12].

### badge

6변형 — outlined-primary(`{colors.primary-60}` 텍스트 + 1px `{colors.primary-50}`), outlined-tertiary(`{colors.fg-2}` + 1px `{colors.border-default}`), solid-primary(흰 텍스트 + `{colors.primary-50}` 배경), solid-pastel(`{colors.primary-70}` + `{colors.primary-10}` 배경), 그리고 semantic success/warning/danger. 모두 `{rounded.radius-xsmall}` (4px), 13px bold. `{component.notice-list}`의 뱃지가 outlined-primary 변형이다 [src:12].

### alert

info / success / warning / danger 배너. 패딩 14/16, `{rounded.radius-medium}` (8px), 1px 동색 보더 + 파스텔 배경 + 24px 아이콘 원, 제목 15px bold `{colors.fg-1}`, 본문 13px `{colors.fg-2}`. 색 앵커는 `{colors.info}` · `{colors.success}` · `{colors.warning}` · `{colors.danger}`. 메시지는 원인과 다음 단계를 함께 적는다 [src:1][src:12].

### critical-alert

재난·긴급 공지 전용 풀폭 배너. `{colors.danger}` 배경 + 흰 텍스트, 32px 아이콘 원(반투명 오버레이), 제목 17px bold + 본문 15px. 한 화면에 하나만, 최상단 고정으로 노출한다 [src:12].

### infobox

info / tips / warning 콜아웃. 패딩 18/20, `{rounded.radius-large}` (10px), 흰 배경 + 1px 보더, 32px 아이콘 원(info `{colors.info}` · tips `{colors.primary-50}` · warning `{colors.warning}`), 제목 17px bold + 본문 15px + 선택 액션 버튼. `{component.alert}`보다 한 호흡 긴 안내·도움말에 쓴다 [src:12].

### bottom-sheet

모바일 하단 시트. 상단 코너는 표준 스케일보다 큰 16px(시트 한정), 36×4 `{colors.gray-30}` grab handle, 항목 패딩 14px. 뒤에 `oklch(0.16 0.003 264 / 0.55)` (gray-100 55%) 스크림, 240ms 슬라이드업 [src:12].

### tooltip · contextual-help

tooltip은 패딩 8/12 `{rounded.radius-small}`, `{colors.fg-1}` 배경 흰 텍스트 13px, `{elevation.shadow-2}`, CSS 삼각 화살표. contextual-help는 인라인 블록으로 `{colors.primary-10}` 배경 + 4px `{colors.primary-50}` 좌측 보더, 헤더 12px bold `{colors.primary-60}` 대문자 [src:12].

### progress-spinner

진행률은 `{colors.gray-20}` 트랙 + `{colors.primary-50}` 채움 바로 표시한다. 스피너는 확인된 백그라운드 작업에만 쓰고, 콘텐츠 로딩은 스피너 대신 스켈레톤 로더로 처리한다 [src:1][src:12].

### coach-mark · user-feedback

coach-mark는 온보딩 말풍선으로 대상 요소를 `{colors.primary-50}`로 강조하며 단계 이동을 제공한다. user-feedback은 페이지 하단의 만족도(별점/예·아니오) 입력 모듈이다 [src:12].

### splash-screen · help-panel · masthead-language

splash-screen은 앱 초기 로딩 전체 화면(seal + 워드마크). help-panel은 우측에서 슬라이드되는 튜토리얼 패널. masthead-language는 KO/EN 언어 전환으로, `{component.utility-row}` 또는 마스트헤드에 배치한다 [src:12].

Figma 원본은 위 컴포넌트 외에도 28종 패턴 레이아웃과 다수 변형을 정의하며, 본 번들은 그중 38개 컴포넌트를 standalone 프리뷰로, 11개 흐름을 패턴 페이지로 구현했다 [src:1][src:8].

## Patterns — 페이지 UI 킷

번들의 `ui_kits/`는 컴포넌트를 실제 정부 서비스 흐름으로 조립한 11개 패턴 페이지를 제공한다 [src:13]. 각 페이지는 위 컴포넌트만으로 구성되며 별도의 일회성 스타일을 더하지 않는다.

- **krds-website** — 정부 포털 홈(identifier strip · masthead · hero · search · service-tile · notice-list · footer) [src:4].
- **login** — 간편인증 / 공동·금융인증서 / 아이디 3-탭 로그인. pill `{component.tabs}` + 인증수단 6-그리드 + 폼 필드 [src:13].
- **signup** — 약관 동의 → 본인인증 → 정보 입력 3-step 회원가입(`{component.step-indicator}`) [src:13].
- **civil-application-form** — 5-step 민원 신청(본인 인증 → 정보 입력 → 발급 옵션 선택 → 신청 내용 확인 → 신청 완료). 좌측 `{component.step-indicator}` + 폼 그리드 + `{component.infobox}` + 요약 [src:13].
- **application-complete** — 신청 완료 화면 + 처리 단계 타임라인 [src:13].
- **board-list / board-detail** — 공지사항 목록(필터 + `{component.pagination}`)과 상세(첨부 + 이전/다음 글) [src:13].
- **mypage** — 마이페이지 대시보드(활동 통계 + 최근 항목) [src:13].
- **customer-center** — 고객센터(검색 + 연락처 + FAQ `{component.accordion}` + 채팅 위젯) [src:13].
- **error-pages** — 404 / 500 / 점검중 오류 상태 [src:13].
- **mobile** — 모바일(390 폭) 홈(퀵 그리드 + 하단 탭 바 + `{component.bottom-sheet}`) [src:13].

이 패턴들은 정부 식별 요소(공식 배너·정부상징 seal·워드마크)와 민원 도메인 흐름(본인 인증·간편인증·공동인증서)을 포함한다. **이들을 비정부 제품에 그대로 이식하지 않는다** — KRDS에서 차용할 것은 접근성 우선의 시각 언어(색 단계·1px 보더 위계·포커스 아웃라인·절제된 모션)이지 정부 서비스의 제품 개념·법적 식별자가 아니다 [src:4][src:5].
...
- 정부 식별 요소(`{component.government-identifier-strip}`의 "공식 전자정부 누리집" 배너, `{component.header-brand-block}`의 대한민국정부 seal·워드마크)와 민원 도메인 흐름(본인 인증·간편인증·공동인증서)을 비정부 제품에 그대로 이식하지 않는다 — KRDS에서 차용할 것은 접근성 우선의 시각 언어(색 단계·1px 보더 위계·포커스 아웃라인·절제된 모션)이지 정부 서비스의 제품 개념·법적 식별자가 아니다 [src:4][src:5].
...
|---|---|---|
| Mobile | ≤ 640px | Display L 47→34px, Heading L 32→24px [src:1]; 단일 컬럼; `{component.utility-row}`는 햄버거 트레이로 이동; `{component.service-tile}` 4-up → 2-up |
...
- 폼 검증·에러 상태는 이제 `{component.input-text}`의 error 상태(`{colors.danger}` 보더 + helper text)로 일부 토큰화되었다 [src:12]. 다만 폼 전체의 인라인 검증 타이밍·에러 메시지 카탈로그는 여전히 패턴 페이지에만 부분 노출된다 [src:13].
- KRDS 표준은 라이트 모드와 **선명한 화면 모드**(high-contrast)를 1등 시민으로 명시한다 [src:1][src:7]. 본 entry의 프리뷰 `dark.html`은 일반 다크 모드가 아니라 이 **선명한 화면 모드**(near-black 표면 사다리 + 리프트된 primary 명도 + 약화 그림자)를 시연하며, 별도의 일반 다크 모드 변형은 KRDS 범위 밖이다.
...
1. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/README.md` (시각 기반, 보이스/톤 규칙, 컴포넌트 분류, 아이콘 규칙)
2. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/colors_and_type.css` (모든 CSS 커스텀 프로퍼티: 정확한 hex 값, 타입·라운드·스페이싱·섀도우 토큰)
3. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/SKILL.md` (보이스/팔레트 앵커 매니페스트)
4. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/ui_kits/krds-website/index.html` (sample portal: identifier strip, header, hero, tiles, notice list, modal, toast)
...
12. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/preview/*.html` (38개 컴포넌트 개별 스펙 프리뷰 라이브러리: table, tabs, alert, critical-alert, form-controls, text-input, select, date-input-calendar, badge, accordion, pagination, breadcrumb, side-navigation, step-indicator, infobox, bottom-sheet, tooltip, chip-tag 등)
13. https://api.anthropic.com/v1/design/h/54YYwKLU1KKucsmB_UcQfQ — Claude Design 번들 `project/index.html` + `project/ui_kits/{login,signup,civil-application-form,application-complete,board-list,board-detail,mypage,customer-center,error-pages,mobile}/index.html` (contact sheet + 11개 페이지 패턴 합성)
