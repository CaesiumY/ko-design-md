...
전체 팔레트는 `colors_and_type.css`의 CSS 커스텀 프로퍼티를 OKLCH로 변환한 값이다. 그레이·프라이머리·세컨더리·액센트의 4계열에 시맨틱(상태) 토큰이 더해지며, 액센트(워밍 레드)는 한 화면의 5% 미만에서만 — 알럿·치명 상태·단일 핵심 강조에 — 사용한다.
...
**Family.** 단일 패밀리 — **Pretendard GOV (정부용 Pretendard)**, Pretendard을 공공 부문 접근성과 한글 가독성에 맞춰 튜닝한 정부 전용 컷을 사용한다. 스택은 다음과 같다:
...
보수적인 5단계 스케일이며 최댓값은 12px이다. 이는 "정부에 적합한 신뢰·안정·친근감을 유희적으로 흐르지 않게 전달하기 위한" 의도적 상한이다.
