...
> **대조 결과(2026-08-02).** 이 문서의 출처 11개는 **전부 블로그 글과 가이드 문서**이고 토큰 패키지나 값 표를 발행하는 곳이 없다 [src:2][src:3][src:5]. 즉 이 절의 색 토큰 37개는 산문 서술 위에 서 있고, 다른 항목처럼 배포물과 값을 1:1로 맞춰 볼 경로가 존재하지 않는다. 카탈로그에서 TDS 값 검증 난도가 높은 이유이며, 값을 그대로 쓰려는 사용자는 실제 토스 서비스에서 렌더값을 확인하는 편이 안전하다.
