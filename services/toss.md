...
`grey-50`~`grey-900` 10단계는 TDS 발행값이다 [src:12][src:13]. `grey-150`은 TDS에 없는 번들 고유 step이라 번들값을 두었고, 그 결과 발행값으로 옮긴 `grey-200`보다 약간 어둡다. `grey-900`은 순수 검정이 아니라 미세하게 차가운 navy 톤이며, `grey-100`은 거의 인지되지 않는 cool 그레이로 화면 보조 표면으로 사용된다.
