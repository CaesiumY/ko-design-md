...
- 학습·참조: 상세 페이지 정독 → References 의 출처 링크로 이동
...
KRDS 등) — 브랜드 발행물이라 권위는 최고지만 형식이 통일돼 있지 않고 LLM 주입용이 아니다.
**이들은 경쟁자이자 우리가 인용하는 출처다** — 대체하는 게 아니라 인용한다.
...
**How we do it differently:** 사람이 눈으로 베끼는 대신, 브랜드 발행물에서 값을 추출하고
인용을 붙여 기계가 검증 가능한 형식으로 고정한다.

**Why that's better:** 값이 틀리면 CI가 잡고, 공개 출처 목록이 비었거나 인용이 그 목록을 벗어나면 머지가 안 된다.
